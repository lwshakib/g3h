...
export function GeminiDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      fetch(`/api/credentials?nodeType=${NodeType.GEMINI}`)
        .then((res) => res.json())
        .then((data) => {
          if (data.credentials) {
  ...
  const handleSave = () => {
    setNodes((nodes) =>
      nodes.map((node) => {
        if (node.id === nodeId) {
          const updatedData: GeminiNodeData = {
            ...(node.data as GeminiNodeData),
            prompt,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Configure the Gemini AI prompt for this node.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
                  ...
                  credentials.map((cred) => (
                    <SelectItem key={cred.id} value={cred.id}>
                      <div className="flex items-center gap-2">
                        <Image src="/logos/gemini.svg" alt="Gemini" width={16} height={16} className="shrink-0" />
                        <span>{cred.name}</span>
          ...
          <div className="grid gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              Select the Gemini model to use for this request
            </p>
          </div>
          ...
          <div className="grid gap-2">
            ...
            <Input
              id="variableName"
              placeholder="geminiResponse (optional - auto-generated if not set)"
              value={variableName}
              onChange={(e) => setVariableName(e.target.value)}
            />
            <p className="text-xs text-muted-foreground">
              Name to store this response (e.g., "geminiResponse"). Other nodes
              can reference it using {"{{geminiResponse}}"}
            </p>
          </div>
        </div>
