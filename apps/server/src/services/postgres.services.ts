...
export class PostgresService {
  ...
  public async updateUser(
    ...
    logger.info(
      `[PostgresService] Updated User ${userId}: ${Object.keys(data).join(", ")}`
    )
    return rows[0]
  }
  ...
  public async createAccount(data: {
    ...
    logger.info(
      `[PostgresService] Created Account: ${data.providerId} for User: ${data.userId}`
    )
    return rows[0]
  }
  ...
  public async updateAccountPassword(email: string, newPasswordRaw: string) {
    ...
    logger.info(
      `[PostgresService] Updated password for credentials account: ${email}`
    )
  }
  ...
  public async createVerification(identifier: string) {
    ...
    logger.info(
      `[PostgresService] Created Verification Token for: ${identifier}`
    )
    return rows[0]
  }
  ...
  public async createWorkflow(data: {
    ...
    logger.info(
      `[PostgresService] Created Workflow: ${data.name} for User: ${data.userId}`
    )
    return rows[0]
  }

  public async createWorkflowWithAutoName(
    userId: string,
    ...
    const existingNames = new Set(
      existingNamesResult.rows.map((row) => row.name as string)
    )
