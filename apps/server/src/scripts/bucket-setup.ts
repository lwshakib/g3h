  ...
  try {
    ...
    try {
      ...
      if (
        ...
        logger.warn(
          `[BucketSetup] ℹ️ Bucket "${bucket}" already exists. Proceeding with configuration.`
        )
      } else {
        throw error
      }
    ...
    try {
      ...
      if (
        ...
        logger.warn(
          `[BucketSetup] ℹ️ Public Access Block not supported by this provider (R2). Skipping.`
        )
      } else {
        throw error
      }
