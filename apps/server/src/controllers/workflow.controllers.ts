...
const runWorkflowSequence = async (
  ...
  const shouldExecuteNode = (nodeId: string) =>
    allowedNodeIds ? allowedNodeIds.has(nodeId) : true
  ...
  const queue: { nodeId: string; inputData: any }[] = [
    { nodeId: manualTrigger.id, inputData: null },
  ]
  ...
  while (queue.length > 0) {
    ...
    if (node.type === "manualTrigger" || node.type === "manual-trigger") {
      ...
      const payload = rawPayload
        ? interpolateString(rawPayload, context)
        : undefined
    ...
    for (const edge of nextEdges) {
      if (!edge.target || visited.has(edge.target) || queued.has(edge.target))
        continue
...
export const createWorkflow = async (req: any, res: Response) => {
  ...
  try {
    const workflow = await postgresService.createWorkflowWithAutoName(
      userId,
      baseName,
      {}
    )
    ...
    logger.error(
      `[WorkflowController] Create workflow failed: ${error.message}`
    )
    res
      .status(500)
      .json({ success: false, message: "Failed to create workflow." })
  }
}
...
export const deleteWorkflow = async (req: any, res: Response) => {
  ...
  try {
    const deleted = await postgresService.deleteWorkflowForUser(
      workflowId,
      userId
    )
    if (!deleted) {
    ...
    logger.error(
      `[WorkflowController] Delete workflow failed: ${error.message}`
    )
    res
      .status(500)
      .json({ success: false, message: "Failed to delete workflow." })
  }
}
...
export const getWorkflowById = async (req: any, res: Response) => {
  ...
  try {
    const workflow = await postgresService.findWorkflowByIdForUser(
      workflowId,
      userId
    )
    if (!workflow) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to load workflow." })
  }
}
...
export const updateWorkflow = async (req: any, res: Response) => {
  ...
  try {
    const updated = await postgresService.updateWorkflowForUser(
      workflowId,
      userId,
      {
        name,
    ...
    logger.error(
      `[WorkflowController] Update workflow failed: ${error.message}`
    )
    res
      .status(500)
      .json({ success: false, message: "Failed to update workflow." })
  }
}
...
export const executeWorkflow = async (req: any, res: Response) => {
  ...
  const workflowId = req.params.id
  const stopAtNodeId =
    typeof req.body?.targetNodeId === "string"
  ...
  try {
    ...
    logger.error(
      `[WorkflowController] Execute workflow failed: ${error.message}`
    )
    res
      .status(500)
      .json({ success: false, message: "Failed to execute workflow." })
  }
}

export const executeWorkflowStream = async (req: Request, res: Response) => {
  const workflowIdRaw = req.params.id
  const workflowId = Array.isArray(workflowIdRaw)
    ? workflowIdRaw[0]
    : workflowIdRaw
  ...
  const stopAtNodeId = Array.isArray(targetNodeIdRaw)
    ? typeof targetNodeIdRaw[0] === "string"
      ? targetNodeIdRaw[0]
      : undefined
    : typeof targetNodeIdRaw === "string"
      ? targetNodeIdRaw
      : undefined
  ...
  if (!workflowId) {
    return res
      .status(400)
      .json({ success: false, message: "Missing workflow id." })
  }

  if (!token) {
    return res
      .status(401)
      .json({ success: false, message: "Missing session token." })
  }
  ...
  try {
    ...
    logger.error(
      `[WorkflowController] Execute workflow stream failed: ${error.message}`
    )
    if (!res.headersSent) {
      return res
        .status(500)
        .json({ success: false, message: "Failed to stream execution." })
    }
    res.write(
