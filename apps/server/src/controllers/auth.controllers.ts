...
export const googleAuth = (req: Request, res: Response, next: NextFunction) => {
  ...
  const state = callbackURL
    ? Buffer.from(callbackURL as string).toString("base64")
    : undefined
...
export const googleCallback = async (req: any, res: Response) => {
  try {
    ...
    const session = await postgresService.createSession(
      user.id,
    ...
    try {
      ...
      logger.error(
        "Failed to send social login email notifications:",
        emailError
      )
    }
...
export const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const existingAccount = await postgresService.findAccount(
      "credentials",
      email
    )
    ...
    if (existingAccount) {
      return res.status(400).json({
        success: false,
        message: "Security Breach: Identity already registered.",
      })
    }
...
export const login = (req: Request, res: Response, next: NextFunction) => {
  passportService.passport.authenticate(
    "local",
      ...
      try {
        if (
          ...
          return res.status(403).json({
            success: false,
            message: "Security Access Denied: Protocol verification required.",
          })
        }
...
export const verifyEmail = async (req: Request, res: Response) => {
  ...
  if (!token)
    return res
      .status(400)
      .json({ success: false, message: "Invalid payload: Token mismatch." })

  try {
    const verification = await postgresService.findVerificationByToken(
      token as string
    )
    if (!verification)
      return res.status(400).json({
        success: false,
        message: "Security Protocol Mismatch: Invalid or expired token.",
      })
    ...
    if (user) {
      ...
      res
        .status(404)
        .json({ success: false, message: "Node Identity not found." })
    }
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message })
  }
...
export const forgotPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message:
        "If the identity exists, reset instructions have been broadcasted.",
    })
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message })
  }
...
export const resetPassword = async (req: Request, res: Response) => {
  ...
  if (!token || !newPassword) {
    return res.status(400).json({
      success: false,
      message: "Invalid payload: Missing token or password.",
    })
  }
  ...
  try {
    ...
    await postgresService.updateAccountPassword(
