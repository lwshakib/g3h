...
export const getUploadUrl = async (req: any, res: Response) => {
  ...
  if (!contentType || !extension) {
    return res.status(400).json({
      success: false,
      message: "Invalid payload: contentType and extension required.",
    })
  }
  ...
  try {
    ...
    logger.error(
      `[UserController] Presigned URL generation failed: ${error.message}`
    )
    res.status(500).json({
      success: false,
...
export const updateProfile = async (req: any, res: Response) => {
  ...
  try {
    const updatedUser = await postgresService.updateUser(userId, {
      name,
      image,
    })
    ...
    res
      .status(500)
      .json({ success: false, message: "Sync_Identity_Protocol_Error" })
  }
}
